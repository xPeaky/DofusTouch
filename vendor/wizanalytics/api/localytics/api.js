...
var LocalyticsSession = function (appKey, options) {
	...
	// Custom code replacing localStorage
	...
	storage.setItem = function (key, data) {
		try {
			...
			log("setItem failed by exception.");
		}

		storageData[key] = data.toString();
	};
	...
	storage.getItem = function (key) {
		...
		try {
			...
			log("getItem failed by exception.");
		}

		if (!storageData.hasOwnProperty(key)) {
			return item;
		}
		return item || storageData[key];
	};
	...
	storage.removeItem = function (key) {
		try {
			...
			log("removeItem failed by exception.");
		}
	...
	return (function (window, undefined) {
		...
		var MAX_UPLOAD_LENGTH = 16000; // Changed from 64000
		 ...
		 */
		var maxLocalStorageSize = 10000; // Changed from 100000
		...
		var generateUserId = function () {
			return options.useCustomerId ? '' : generateUUID();
		};

		var getInstallUUID = function () {
			// TODO: Hash app uuid as part of device id
			installUUID = ref._getOrCreateProperty(installUUID, INSTALL_UUID, generateUserId, false);
			return installUUID;
		};
		...
		ref._tagScreen = function (screenName) {
			...
			// limit the flow to x screens
			if (len < screenLimit && screenFlows[len - 1] !== name) {
		...
		ref._setCustomerId = function (customerId) {
			...
			if (options.useCustomerId) {
				installUUID = customerId;
				ref._writeStore(INSTALL_UUID, installUUID);

				// make sure the header is set with the correct install UUID
				getHeader(true);
				//TODO: if customerID differ from installUUID in storage, reset the session
			}
		};
...
module.exports = LocalyticsSession;


/*****************
 ** WEBPACK FOOTER
 ** ./~/wizanalytics/api/localytics/api.js
 ** module id = 167
 ** module chunks = 0
 **/